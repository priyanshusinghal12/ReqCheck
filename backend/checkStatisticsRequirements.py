...
def check_statistics_requirements(student_courses, degree_requirements):
    ...
    if len(remaining_stat_400) >= 1:
        satisfied_requirements.append(["STAT 400-level additional"])
        used_courses.add(next(iter(remaining_stat_400)))
    elif has_advanced_cs:
        satisfied_requirements.append(["Advanced CS Course"])
    ...
    # ✅ Requirement: Complete 3 additional courses from the same departments
    remaining_valid_courses = valid_courses - used_courses
    if len(remaining_valid_courses) >= 3:
        satisfied_requirements.append(["3 additional courses"])
        used_courses.update(list(remaining_valid_courses)[:3])
    else:
...
print(json.dumps(statistics_results, indent=4))  # No used_courses in output
